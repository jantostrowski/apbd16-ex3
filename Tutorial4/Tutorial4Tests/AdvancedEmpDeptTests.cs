...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, e => e.Mgr, y => y.EmpNo,
            (e, y) => new {Employee = e.EName, Manager = y.EName}).ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, 
            ...
            .SelectMany(joined => grades
                .Where(grade => joined.emp.Sal >= grade.Losal && joined.emp.Sal <= grade.Hisal)
                .Select(g => new {joined.emp.EName, joined.dept.DName, g.Grade})).ToList();
